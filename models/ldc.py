...
class PromptLearner(nn.Module):
    def __init__(self, args):
        ...
        self.adapter = nn.ModuleDict({
            "adapter_1": nn.Sequential(nn.Linear(n_cls, ctx_dim, bias=False), nn.BatchNorm1d(ctx_dim),
                                       nn.ReLU(inplace=True), nn.Linear(ctx_dim, ctx_dim, bias=False)),
            "adapter_2": nn.Sequential(nn.Linear(ctx_dim, ctx_dim, bias=False), nn.BatchNorm1d(ctx_dim),
                                       nn.ReLU(inplace=True), nn.Linear(ctx_dim, ctx_dim, bias=False)),
            "adapter_3": nn.Sequential(nn.Linear(ctx_dim, ctx_dim, bias=False), nn.BatchNorm1d(ctx_dim),
                                       nn.ReLU(inplace=True), nn.Linear(ctx_dim, n_cls, bias=False)),
            "g_weight": nn.Sequential(nn.Linear(ctx_dim, ctx_dim, bias=False), nn.BatchNorm1d(ctx_dim),
                                      nn.ReLU(inplace=True), nn.Linear(ctx_dim, 1, bias=False), nn.Sigmoid()),
            "mlp_head": nn.Sequential(nn.Linear(ctx_dim, ctx_dim, bias=False), nn.BatchNorm1d(ctx_dim),
                                        nn.ReLU(inplace=True), nn.Linear(ctx_dim, n_cls, bias=False))
        })
    ...
    def forward(self, audio_features, text_features):
        ...
        _clap_logits = audio_features @ text_features.t()
        clap_logits = 100.0 * _clap_logits
     
        ada_logits = clap_logits + self.adapter["adapter_3"](
            self.adapter["adapter_1"](clap_logits) + self.adapter["adapter_2"](audio_features))

        alpha = self.adapter["g_weight"](audio_features)
        mlp_logits = self.adapter["mlp_head"](audio_features)
        total_logits = (1-alpha) * ada_logits + alpha * mlp_logits
...
class CustomPENGI(nn.Module):
    ...
    def forward(self, audio):
        ...
        prompts = [f"a audio of {class_name}" for class_name in self.args.classnames]
        ...
        logits = self.prompt_learner(audio_features, text_features) # text_features shape [n_text_prompts, 1024]
