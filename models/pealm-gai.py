...
class CustomPENGI(nn.Module):
    ...
    def forward(self, audio, label, mode='test'):
        ...
        if mode == 'test':
            text_proto, visual_proto = self.prompt_learner(audio_features, label, mode)
            text_proto = text_proto / text_proto.norm(dim=-1, keepdim=True)
            visual_proto = visual_proto / visual_proto.norm(dim=-1, keepdim=True)
            text_logits = audio_features @ text_proto.t()
            audio_logits = audio_features @ visual_proto.t()
            mu = torch.mean(text_logits, dim=-1, keepdim=True)
            sigma = torch.std(text_logits, dim=-1, keepdim=True)
            normalized_logits = (text_logits - mu) / sigma
            moment_4 = torch.mean(normalized_logits ** 4, dim=-1, keepdim=True)
            return  text_logits + 1 / ((moment_4 / 250) ** 4) * audio_logits
