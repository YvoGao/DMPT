...
class Prototype_Evolution(nn.Module):
    def __init__(self, args, text_features):
        ...
        self.text_proto= self.text_proto = nn.Parameter(text_features, requires_grad=False)
        ...
        # self.ctx = torch.nn.Parameter(text_proto)
        reduction = 2
        self.adapter =  nn.Sequential(
            nn.Linear(text_features.shape[1], text_features.shape[1] // reduction, bias=False),
            nn.ReLU(inplace=True),
            nn.Linear(text_features.shape[1] // reduction, text_features.shape[1], bias=False),
            nn.ReLU(inplace=True)
        ).to(args.device)
    ...
    def forward(self, audio, label, mode='train'):
        ...
        if mode=='init':    
            for ul in self.classes:
                ...
                # 更新视觉原型
                if self.visual_features[ul.item()].shape[0] > 0:
                    # import pdb
                    # pdb.set_trace()
                    self.visual_proto[ul.item()] = torch.tensor(self.visual_features[ul.item()]).to(self.args.device).mean(dim=0)
                # import pdb; pdb.set_trace()
            self.visual_proto = self.visual_proto / self.visual_proto.norm(dim=-1, keepdim=True)
            audio = audio/audio.norm(dim=-1, keepdim=True)
            zs_logits = audio @ self.text_proto.t()
            audio_features = self.adapter(audio)
            audio_features = audio_features / audio_features.norm(dim=-1, keepdim=True)
            tip_logits = audio_features @ self.visual_proto.t()
            logits = zs_logits + tip_logits
            return logits
        else:
            audio = audio/audio.norm(dim=-1, keepdim=True)
            zs_logits = audio @ self.text_proto.t()
...
class CustomPENGI(nn.Module):
    ...
    def forward(self, audio, label, mode='test'):
        ...
        # audio_features = audio_features/audio_features.norm(dim=-1, keepdim=True)
        return self.prompt_learner(audio_features, label, mode)
...
class Prototype_Evolution(nn.Module):
    def __init__(self, args, text_features):
        ...
        self.classes = np.arange(len(args.classnames))
        self.n_cls = len(args.classnames)
        self.text_proto = nn.Parameter(text_features, requires_grad=False)
        self.visual_proto = torch.zeros(text_features.shape[0], text_features.shape[1]).to(args.device)
        # 创建一个字典，其中每个键对应一个空的 NumPy 数组
        self.visual_features = {cls_name: np.empty((0, text_features.shape[1])) for cls_name in self.classes}
