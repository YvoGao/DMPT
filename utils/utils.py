...
METHODS = ['zeroshot', 'coop', 'cocoop', 'palm', 'DMPT', 'moaalm']
# SHOT = 1

def get_model(args, pengi, models):
    ...
    elif args.model_name == 'DMPT':
        model = models.DMPT(args, pengi)
    elif args.model_name == 'pnalm':
        model = models.PNALM(args, pengi)
    elif args.model_name == 'moaalm':
        model = models.MOAALM(args, pengi)
    elif args.model_name == 'ldc':
        model = models.LDC(args, pengi)
    elif args.model_name == 'claps':
        model = models.CLAPS(args, pengi)
    elif args.model_name == 'frealm':
        model = models.FREALM(args, pengi)
...
def save_model(args, model, save_model_path):
        ...
        # if args.model_name in ['pealm', 'pnalm', 'moaalm']:
        #     torch.save(model.context_learner.state_dict(), save_model_path)
        else:
            raise ValueError(f"Model '{args.model_name}' is not supported. Choose from: [{', '.join(METHODS)}]")
...
def setup_logging(args):
    log_dir = os.path.join('ab_logs{}shot'.format(args.num_shots), args.model_name) # log file dir
